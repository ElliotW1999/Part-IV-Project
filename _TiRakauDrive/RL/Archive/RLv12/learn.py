...
if (len(timeLoss) > 833 and numpy.mean(timeLoss) < 60 and numpy.mean(timeStopped) < 40) or (len(timeLoss) > 790 and numpy.mean(timeLoss) < 57 and numpy.mean(timeStopped) < 38) :    
    for i in range (0,10):
        print('LESHGO')
else:  
    ...
    i = 0 
    while i < len(SARS)-1:
        SARS[i][2] = 10*(nextMoveReward[i] - SARS[i][2])
        i += 1
