...
meanSpeed = meanSpeed / len(stepCosts)
tripdata = minidom.parse("tripinfoRL.xml")
timeLoss = []
timeStopped = []
...
#if (meanSpeed > 8.25) or (numpy.mean(timeLoss) < 51 and numpy.mean(timeStopped) < 34 and len(timeLoss) > 789): 
if meanSpeed > 7.9 or ((numpy.mean(timeLoss) < 54) and (numpy.mean(timeStopped) < 36) and len(timeLoss) > 789):   
    for i in range (0,10):
        print('LESHGO')
else:
