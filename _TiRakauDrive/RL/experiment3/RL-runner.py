...
# needs work
def generate_routefile(seedVal):
    random.seed(int(seedVal))  # make tests reproducible
...
def run(isTesting):
    ...
    while step <= 600:
        ...
        #thirdDeadline_s4219 = detector_delay_s4219.index(detector_delay_s4219_sorted[-3])
        ...
        #if thirdDeadline_s4219 in lanesForGroups_s4219[group]:
        #    thirdDeadlineGroup_s4219 = group
        
        testVariableRename = min(timeInPhase, 8)
        currentState = str(testVariableRename) + str(next_phase_s4220) + str(secondDeadlineGroup_s4219) + str(carsFlowing) + str(currentActivePhase) + str(earliestDeadlineGroup_s4219)  
